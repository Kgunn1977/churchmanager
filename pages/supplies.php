<?php
$pageTitle = 'Supplies — Church Facility Manager';
...
<div class="cat-page">
    <div class="cat-hdr">
        <div>
            <h1>Supplies</h1>
            <p>Consumable items used in cleaning and maintenance tasks.</p>
        </div>
...
<div class="cat-modal" id="catModal">
    <div class="cat-modal-inner">
        <h3 id="modal-title">Add Supply</h3>
        <input type="hidden" id="modal-id">
        <div style="margin-bottom:12px;">
            <label>Name *</label>
            <input id="modal-name" type="text" placeholder="e.g. Paper Towels">
        </div>
        ...
        <button class="btn-delete" id="modal-del" onclick="deleteItem()" style="display:none;">Delete Supply</button>
    </div>
</div>
...
async function apiGet(action) {
    const r = await fetch('/api/supplies_api.php?action=' + action);
    return r.json();
}
async function apiPost(action, data) {
    const r = await fetch('/api/supplies_api.php', {
...
function render() {
    ...
    if (!items.length) {
        el.innerHTML = '<div class="cat-empty"><p style="font-size:14px;font-weight:500;">No supplies yet</p><p style="font-size:12px;">Click "+ Add Supply" to get started</p></div>';
    ...
    `).join('');
    el.innerHTML = html;
    document.getElementById('cat-count').textContent = items.length + ' supply item' + (items.length !== 1 ? 's' : '');
}

function openModal(id) {
    const item = id ? items.find(i => i.id === id) : null;
    document.getElementById('modal-title').textContent = item ? 'Edit Supply' : 'Add Supply';
...
async function deleteItem() {
    const id = document.getElementById('modal-id').value;
    if (!id || !confirm('Delete this supply?')) return;
